...
if (generateForm) {
    generateForm.addEventListener('submit', async (e) => {
        ...
        // 文字提取放在浏览器：Cloudflare Workers 免费版限 10ms CPU/次调用，
        // PDF 解析是纯计算，放服务端会被掐断。提取完只把纯文本发上去。
        let extracted;
        try {
            const { extractText } = await import('/frontend/js/common/doc-extract.js');
            extracted = await extractText(fileInput.files[0]);
            progressText.textContent = `已提取 ${extracted.text.length} 字，正在制作书卷...`;
        } catch (err) {
            progressDiv.style.display = 'none';
            console.error('文档解析失败:', err);
            alert(err.message || '文档解析失败');
            return;
        }
