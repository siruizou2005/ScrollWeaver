...
CHARACTER_PROFILES = {
    ...
    "TyrionLannister": {
        ...
        "detailed_profile": {
            ...
            "detailed_bio": "凯岩城公爵泰温·兰尼斯特的幼子，因患侏儒症被称为\"小恶魔\"（The Imp）。尽管受尽家族（尤其是父亲和姐姐）的鄙视，他却拥有维斯特洛大陆最顶尖的头脑。他嗜书如命，善于洞察人心与政治博弈。他在黑水河之战中挽救了君临，后流亡厄索斯并成为丹妮莉丝的国王之手。他是书中内心最丰富、最具韧性的角色之一。"
        },
        "personality_profile": {
            "core_traits": {
...
class InterlocutorSimulator:
    ...
    def generate_response(self, turn_num, char_role_name, char_response, history, char_data=None):
        ...
        Args:
            turn_num: 当前轮次（0-indexed）
            char_role_name: 角色名称
            char_response: 角色刚才的回复
            history: 对话历史
            char_data: 角色数据（用于生成冲突性话题）
        """
        # 判断是否需要在关键轮次引入话题转换或冲突
...
class DynamicState:
    def __init__(self, mood="neutral", energy=80):
        self.mood = mood
        self.energy = energy
        self.relationships = {} # 关系图谱: {entity_name: intimacy_score}

    def update(self, user_input=None, language="zh", model=None, tokenizer=None):
        """更新心情、能量和关系状态，遵循消极偏见原则"""
        mood_change = "neutral"
        ...
        # 3. 能量更新 (Energy Update) - 论文参数
        # 负面事件(-15)的影响大于正面事件(+10)
        ...
        # 4. 关系更新 (Relationship Update) - 论文参数
        # 假设当前对话对象是 "User" (实际应用中应传入 entity ID)
        target_entity = "User"
        current_intimacy = self.relationships.get(target_entity, 50) # 默认亲密度50
        
        # 亲密度非对称更新: 好事+5, 坏事-3 (论文参数可能略有不同，此处采用非对称逻辑)
        # 论文 3.1.3: delta+ = +5, delta- = -3
...
def generate_dual_process(model, tokenizer, user_input, history, char_data, state, language="zh", interlocutor_name=None, interlocutor_role_name=None, char_key=None, is_action=True):
    """PersonaForge双系统处理：改进上下文和 Catchphrase 处理"""
    p = char_data["personality_profile"]
    c = p["core_traits"]
    s = p["speaking_style"]
...
Write a short First Person ("I") Inner Monologue. NO psychological terms."""
    ...
    # --- Phase 2: 风格化回复 ---
    # 改进 Catchphrase 处理：作为语气参考，严禁生搬硬套
...
def run_experiment(target_groups=None, target_chars=None, num_turns=30, use_dynamic_interlocutor=False):
    ...
    Args:
        ...
        target_chars: 目标角色列表
        num_turns: 对话轮数
        use_dynamic_interlocutor: 是否使用动态对话者模拟（True=使用模型生成，False=使用静态提示列表）
    """
    ...
    for group in groups:
        ...
        for char_key in chars:
            ...
            for interlocutor_key in possible_interlocutors:
                ...
                # 构建输出文件名，包含对话者信息
                if interlocutor_key:
                    output_file = f"{RESULTS_DIR}/{group}_{char_key}_{interlocutor_key}.json"
                else:
                ...
                if os.path.exists(output_file):
                    with open(output_file, 'r') as f:
                        if len(json.load(f).get("logs", [])) >= num_turns:
                            print(f"跳过 {char_key} [{group}] 与 {interlocutor_role_name or '无对话者'} - 已完成")
                            continue
                
                print(f"Running {char_key} [{group}] 与 {interlocutor_role_name or '无对话者'}...")
                ...
                if use_dynamic_interlocutor and interlocutor_key and interlocutor_role_name:
                    ...
                    print(f"  使用动态对话者模拟（关键轮次：{interlocutor_simulator.critical_turns}）")
                
                # 准备场景：如果使用动态对话者，第一轮使用初始提示，后续动态生成
                ...
                if use_dynamic_interlocutor and interlocutor_simulator:
                    # 动态模式：第一轮使用初始提示，后续轮次将动态生成
                    # 第一轮使用 casual prompt 作为开场
                    if len(casual_p) > 0:
...
{detailed.get('detailed_bio', char_data.get('bio', ''))}"""
                ...
                for i, scene in enumerate(scenarios):
                    ...
                    if scene.get("dynamic", False) and interlocutor_simulator and len(history) > 0:
                        ...
                        is_action = False # 对话者生成的是"说话"
                    else:
                        prompt = scene["p"]
                        is_topic_shift = False
                        # 初始 Scenarios 中如果是 casual/stress prompt 通常是 action
                        # 除非显式修改，这里保持默认 True
