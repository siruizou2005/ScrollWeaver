"""
Full Scale 4-Way Long Dialogue Experiment V3 (High-Fidelity PersonaForge Reproduction)

Updates from V2:
1. **LLM-based State Update**: Replaced brittle keyword matching with Gemini-based sentiment analysis for robust Mood/Energy transitions.
2. **Contextualized Inner Monologue**: Phase 1 (Thinking) now sees conversation history, preventing hallucinations in multi-turn dialogues.
...
RESULTS_DIR = "experiments/sft/results/long_dialogue_full_v3"
EVAL_MODEL = "gemini-2.5-flash-lite"

# --- Character Data (Same as V2) ---
CHARACTER_PROFILES = {
    # ========== 红楼梦 ==========
    "LinDaiyu": {
        "role_name": "林黛玉",
    ...
    "ZhugeLiang": {
        ...
        "casual_prompts": ["丞相今日可有计策？", "天象有何异动？", "丞相觉得当前局势如何？", "可否请教丞相治国之道？", "丞相今日可愿抚琴一曲？"]
    },
    "CaoCao": {
        "role_name": "曹操",
...
# --- Dynamic State Manager V3 (LLM-Based) ---
...
class DynamicState:
    ...
    def analyze_sentiment_llm(self, api_key, user_input, language="zh"):
        """Use Gemini to analyze sentiment more accurately"""
        if not api_key: return "neutral" # Fallback
        
        if language == "zh":
            prompt = f"分析这句话对角色的情感倾向(positive/negative/neutral): '{user_input}'。只输出一个单词。"
        else:
...
def evaluate_pc_score(api_key, response, pf_profile, language="zh"):
    ...
    if language == "zh":
        prompt = f"评估回复一致性(0.0-1.0)。特质:MBTI={core.get('mbti','?')},BigFive=[{bf}]。回复:{response}。只输出数字。"
    else:
        prompt = f"Rate consistency(0.0-1.0).Traits:MBTI={core.get('mbti','?')},BigFive=[{bf}].Response:{response}.Output number only."
...
def run_experiment(target_groups=None, target_chars=None, num_turns=50):
    ...
    groups = target_groups if target_groups else all_groups
        ...
        for char_key in chars:
            ...
            for i, scene in enumerate(scenarios):
                ...
                if group == "GroupC_StructuredPrompt":
                    # Dual Process + State Update (V3: With Context and LLM Sentiment)
                    response, monologue = generate_dual_process(current_model, tokenizer, prompt, history, char_data, state, lang)
                    state.update(api_key, prompt, lang) # V3: Pass API Key
                else:
                ...
                if (i+1)%10==0: print(f"  Turn {i+1} | PC={pc} | Avg={sum(pc_scores)/len(pc_scores):.2f}")
