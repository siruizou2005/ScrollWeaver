...
def call_gemini_rest(api_key: str, model: str, system_prompt: str, user_prompt: str) -> str:
    # Using the exact same model and logic as evaluate_only.py
...
def run_eval():
    ...
    output_file = "experiments/sft/results/four_way_comparison_scored.json"
    ...
    # Check if data is nested under "raw_responses" or direct
    ...
    # Model used in evaluate_only.py
    TARGET_MODEL = "gemini-2.5-flash-lite"

    print(f"Starting evaluation using model: {TARGET_MODEL}")
    
    for method in results.keys():
        if method == "aggregate_scores": continue # Skip scores if they already exist
        ...
        for char_key, responses in results[method].items():
            ...
            for item in responses:
                if char_key not in CHARACTERS:
                    print(f"  Warning: Character {char_key} not in CHARACTERS map. Skipping.")
                    continue
                ...
                try:
                    ...
                    print(f"  - Scen {count}: Failed parse. Response: {eval_resp[:50]}...")
