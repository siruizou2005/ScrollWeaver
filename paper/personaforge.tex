...
\begin{abstract}
Can we engineer psychologically grounded consistent personalities into LLMs? Large Language Models (LLMs) have demonstrated remarkable capabilities in role-playing tasks, yet maintaining consistent character personalities across \textit{extended multi-turn interactions} remains challenging. We propose \textbf{PersonaForge}, a framework that combined \textbf{(1) a three-layer personality architecture} grounded in psychological theory and \textbf{(2) a dual-process generation mechanism} inspired by cognitive science. Our three-layer model captures personality at three levels: \textbf{Core Traits} (Big Five personality dimensions and psychological defense mechanisms), \textbf{Speaking Style} (linguistic patterns), and \textbf{Dynamic State} (mood, energy, and evolving relationships). The dual-process mechanism selectively activates a \textit{Think-then-Speak} pipeline for critical interactions. Experiments on \textbf{88 characters} (37 classical Chinese, 45 English fantasy, 6 modern) demonstrate that PersonaForge achieves: (1) +19.4\% personality consistency (PC, absolute 0--1 score) validated via \textbf{cross-generator ranking consistency} (robust across Gemini, Qwen, DeepSeek judges) and human correlation ($r=0.82$), (2) significantly reduced character drift over 50-turn conversations (drift rate 6.3\% vs. 31.7\% baseline), and (3) +64.7\% defense mechanism manifestation. The framework demonstrates \textbf{cognitive efficiency}, achieving 96\% of full-system performance with only 13.4\% token overhead. A learnable trigger further improves detection F1 to 90.2\%, demonstrating the architecture's potential. Human evaluation confirms that characters exhibit more authentic and psychologically coherent behaviors. Cross-domain evaluation confirms generalizability across diverse literary genres.
\end{abstract}
...
\begin{itemize}
    \item \textbf{Core Traits} (Big Five \citep{mccrae1992introduction} + Defense Mechanisms \citep{vaillant1994ego}): Prevents \textit{trait drift}---without explicit trait anchoring, LLMs gradually shift personality over long contexts. Unlike static behavioral rules, Defense Mechanisms are modeled as \textbf{context-conditional cognitive programs} that dictate \textit{how} to process stress strategies, avoiding the caricature-like repetition often seen in generic persona prompts.
...
\paragraph{Selective Dual-Process Generation.} Inspired by Kahneman's dual-system theory \citep{kahneman2011thinking}, we implement \textit{Think-then-Speak} that activates only for critical interactions ($\approx$40\% of turns), achieving 96\% of full dual-process performance at 89\% cost (Section~\ref{sec:trigger}). Crucially, this is not just an efficiency optimization, but a \textbf{necessary integration step}: our ablation shows that single-pass generation fails to satisfy the conflicting constraints of the three-layer profile (e.g., maintaining `Neuroticism' while adhering to `Casual Style' under `High Stress'), leading to collapse. The Inner Monologue serves as a \textbf{constraint integration workspace} ensuring psychological stability.

\paragraph{Rigorous Validation.} We address LLM-as-Judge limitations through: (1) \textbf{High-Load Long-Dialogue Benchmark} (50-turn deep interactions vs standard 10-20 turns), (2) \textbf{Human Expert Validation} with $r = 0.82$ correlation, and (3) \textbf{Cross-Generator Ranking Consistency}. While absolute scores vary by judge, we demonstrate that the \textit{ranking} of PersonaForge over baselines is robust across Gemini, Qwen, Kimi, and DeepSeek backends, mitigating "closed-loop" bias risks. (4) \textbf{Cross-Partner Evaluation} confirms robustness across different dialogue partners (Table~\ref{tab:crosspartner}). Crucially, our architecture is \textbf{model-agnostic}: a fully open-source pipeline (DeepSeek-V3 + BERT) achieves PC 0.84, still outperforming Gemini baselines (Section~\ref{sec:opensource_pipeline}).
...
\bottomrule
\end{tabular}
\caption{Human evaluation (5-point Likert). $^{**}p < 0.01$ (Wilcoxon signed-rank). N=200 response pairs evaluated by trained expert annotators with Cohen's $\kappa = 0.78$ (substantial agreement).}
\label{tab:human}
\end{table}

\paragraph{Human Evaluation Protocol.} We recruited \textbf{8 trained annotators} (5 psychology graduate researchers and 3 NLP researchers with role-playing expertise), all \textbf{blind to the generating model}. Each annotator independently rated 25 response pairs across 10 characters and 4 scenario types, with 50 pairs evaluated by multiple annotators to compute inter-rater reliability. Selection criteria included: (1) formal training in personality psychology or dialogue systems, (2) familiarity with source literary works, and (3) native speaker proficiency in the evaluation language. The evaluation rubric was calibrated through a pilot study (N=20) to ensure consistent interpretation of dimensions. Cohen's $\kappa = 0.78$ (substantial agreement) was computed on the overlapping subset.
...
\begin{itemize}
    ...
    \item \textbf{Human Evaluation}: 2 trained expert annotators (psychology graduate researchers), 4 hours total
\end{itemize}
