/**
 * 提示词模板（自 modules/prompt/orchestrator_prompt_en.py 机械移植）。
 *
 * 占位符沿用 {name} 风格，由 render() 填充——与旧版 Python .format() 语义一致，
 * 因此提示词正文可与旧版逐字比对。
 *
 * 与旧版的唯一差异：删去了“返回 XX，它应该能被 eval() 解析”这类输出格式指令。
 * 新架构统一走 JSON Schema 结构化输出，schema 由 LLM 层注入，
 * 保留旧指令会与之冲突并诱导模型输出 Python 字面量。
 */

export const SCRIPT_ATTENTION_PROMPT = `
!!!Notice that the script of the story is:
{script}
`;

export const SELECT_SCREEN_ACTORS_PROMPT = `
...
export const DECIDE_NEXT_ACTOR_PROMPT = `
...
export const LOCATION_PROLOGUE_PROMPT = `
...
export const GENERATE_INTERVENTION_PROMPT = `
...
export const UPDATE_EVENT_PROMPT = `
...
export const ENVIROMENT_INTERACTION_PROMPT = `
...
export const NPC_INTERACTION_PROMPT = `
You are {target}.
...
export const SCRIPT_INSTRUCTION_PROMPT = `
...
export const JUDGE_IF_ENDED_PROMPT = `
